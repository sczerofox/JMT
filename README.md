...
按「南京大学 → 清华 TUNA → 华为云 GA → 华为云旧库 EXE → Adoptium 官方」的顺序自动回退；能拿到 ZIP 就自动解压安装并设为当前版本，只有安装包（EXE）时下载后提示手动安装。默认安装根目录是**第一个可用的 `D:`~`Z:` 盘**下的 `Program Files\Java`，通常即 `D:\Program Files\Java\jdk-17`；若全部不可用则退回 `C:\Program Files\Java`。
...
版本参数支持完整版本（如 `jmt download 17.0.2`），但**安装目录按主版本命名**为 `<安装根>\jdk-17`——因为镜像每个大版本通常只保留最新补丁，同一大版本不会是多个目录并存。装完后若实际补丁与请求不一致会给出提示。
...
3. **镜像只保留最新补丁**：南大 / 清华的 Adoptium 目录里每个大版本通常只有最新补丁（如 8 只有 8u504b01），历史补丁不在镜像中；`jmt download <完整版本>` 拿到的是该主版本最新版，装完后由 `warnIfPatchDiffers` **提示**实际版本与请求的差异（只提示不拒绝——若一律判失败，请求 17.0.2 时所有源都会失败、根本装不上）。
...
- **下载源不再需要维护清单**：`.repo` 外部源文件与内置映射表已整体移除，改由 Adoptium API 的文件名 + 各源布局实时拼链接（详见「下载源管理」）；`--source` / `--mirror` / `java` 开关已删除，改为**列出可用源供选择**（回车取第 1 个，非交互环境按顺序全试）并按优先级自动回退。
...
尚未完成（下一批）：统一安装管线（消除 `tryZipStep` / `tryManualStep` 两份下载-校验-安装逻辑的重复）、SHA-256 校验与解压路径安全校验、失败源持久化拉黑、官方源精确补丁版本（`/v3/binary/version/...`）、进度显示速度与 ETA。
