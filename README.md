...
下载前会列出该版本可用的源，并让你挑一个（**在交互式终端里**）：

```
[INFO] 目标版本: 6

[INFO] 查找到可用源：2（候选链接共 6 条）

[INFO] 请选择要使用的源：
[INFO]             1. 华为云镜像（Oracle 旧版）（3 条链接）
[INFO]             2. 南京大学镜像（2 条链接）
[INFO]             3. Adoptium 官方源

[INFO] 请输入您选择的源（回车默认选择: 1 ）：

[INFO] 尝试源（源 1/6）：华为云镜像（Oracle 旧版）
[INFO] 下载链接：https://repo.huaweicloud.com/java/jdk/6u45-b06/jdk-6u45-windows-x64.exe
[INFO] 正在下载 EXE 安装程序...
```

- **源名**按主机自动识别（华为云 / 南京大学 / 清华 TUNA / 中科大 / 阿里云 / Adoptium 官方 / 本地文件…），未知主机直接显示主机名；
- **同主机的多条链接算同一个源**（所以「候选链接」数可能大于「源」数），选中后会把该源的链接排到最前；
- 所选源失败时**仍会回退**到其余源，不会因为挑了一个临时不可用的镜像而整体失败；
- 回车 = 使用第 1 个源；`--source 2`（或 `--source=2`）可直接指定、不弹交互；
- **非交互环境**（stdin 被重定向，脚本/CI）不会弹提示，自动按顺序尝试全部源。
...
[INFO] 下载中: 25%    已下载 47.6 MB    已用时 5 秒
[INFO] 下载中: 50%    已下载 95.2 MB    已用时 11 秒
[INFO] 下载中: 61%    已下载 116.1 MB    已用时 15 秒
[INFO] 下载完成: 190.4 MB，平均 8.7 MB/s           ← 结束后汇总（字节数/平均速度）
```

- 实时进度行（不带 `[INFO]` 前缀的那条）在**真实控制台**里原地刷新；输出被重定向到文件时它不会出现，但**里程碑行（每 25%）与每 10 秒一次的普通进度行**都会写入文件，因此日志里也能确认下载在推进。
- 两种模式不会同时出现：控制台里只保留原地刷新那一行（不重复刷 `[INFO]` 行）；重定向/管道里只保留 `[INFO]` 进度行。
