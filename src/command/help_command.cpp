...
ExitCode HelpCommand::execute(const std::vector<std::wstring>& args, AppContext& ctx) {
    if (args.size() > 1) {
        ...
        ctx.out->line(OutputLevel::Info, L"  search             扫描并自动设置 JDK（--force/--user/--sys）");
        ctx.out->line(OutputLevel::Info, L"  list               列出已安装版本");
        ctx.out->line(OutputLevel::Info, L"  use <版本号>       切换当前版本（--exact/--user/--sys）");
        ctx.out->line(OutputLevel::Info, L"  download <版本号>  从镜像/官方源下载并自动配置（--mirror/exe/java/--user/--sys）");
        ...
        ctx.out->line(OutputLevel::Info, L"  env                注册 JMT 目录到系统 PATH（--user/--sys）");
