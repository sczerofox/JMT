#include "jdk_scan_service.hpp"
#include "../infrastructure/file_lock.hpp"
#include "../utils/utils.hpp"
#include "../utils/string_helper.hpp"
#include "../print/console_progress.hpp"
...
static void ScanDirectoryRecursive(const std::wstring& path, int depth, int maxDepth,
    ...
    if (JdkScanService::isValidJdk(path)) {
        ...
        if (!ver.empty() && seen.find(ver) == seen.end()) {
            ...
            found++;
            // 如果您希望看到实时进度，取消下一行的注释
            // ConsoleProgress::ShowScanning(drive, found);
        }
    }
